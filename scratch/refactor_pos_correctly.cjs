...
// Inject renderCartFn right before "return (" (line 643)
let newContent = beforeCart;
const returnIdx = newContent.indexOf("\n  return (");
if (returnIdx === -1) {
...
// Inject the showMobileCart state right after the state declarations
// Let's look for "const [posOrderTab" and insert the state right after it
const stateSearchStr = 'const [posOrderTab, setPosOrderTab] = useState<"all" | "pending" | "preparing" | "ready" | "archived">("all");';
const stateIdx = newContent.indexOf(stateSearchStr);
if (stateIdx !== -1) {
  const beforeState = newContent.substring(0, stateIdx + stateSearchStr.length);
  const afterState = newContent.substring(stateIdx + stateSearchStr.length);
  ...
  newContent = beforeState + stateInjection + afterState;
}
...
console.log("POSDashboardView.tsx refactored correctly!");
